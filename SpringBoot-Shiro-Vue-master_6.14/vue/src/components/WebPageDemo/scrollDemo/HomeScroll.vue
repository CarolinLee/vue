...
<script>
  export default {
    ...
    mounted() {
      ...
      //点击向右事件
      scrollRight.childNodes[pointIndex].onclick = function () {
        pointIndex++;
        scrollPoint.childNodes[pointIndex-1].classList.remove("current");
        if (pointIndex > 2) {
          pointIndex = 0;
        }
        scrollBox.style.left = -1400 * (pointIndex) -40 + "px";
        scrollPoint.childNodes[pointIndex].classList.add("current");
        console.log(pointIndex)
      }

      //点击向左事件
      scrollLeft.childNodes[pointIndex].onclick = function () {
        scrollPoint.childNodes[pointIndex].classList.remove("current");
        pointIndex--;
        if (pointIndex < 0) {
          pointIndex = 2;
        }
      ...
      // for(let i = 0; i < scrollRight.childElementCount; i++){
      //   scrollRight.childNodes[i].onclick = function () {
      //     for(let j = 0; j < scrollRight.childElementCount; j++){
      //       scrollPoint.childNodes[j].classList.remove("current");
      //     }
      //     scrollPoint.childNodes[i].classList.add("current");
      //     pointIndex = i;
